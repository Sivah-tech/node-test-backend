...
export const queryBuilder = (payload: Payload, querySearchKeyInBackend = ['text']) => {
    // Extract 'query' from payload instead of 'description'
    let { query = '', order = '', orderColumn = '' } = payload;
    
    // If 'query' is provided, build the query using regex on the specified backend search keys
    const queryObj = query ? { $or: querySearchKeyInBackend.map(key => ({ [key]: { $regex: query, $options: 'i' } })) } : {};
    
    // If order and orderColumn are provided, build the sorting object
    const sort: { [key: string]: SortOrder } = order && orderColumn ? { [orderColumn]: order === 'asc' ? 1 : -1 } : {};

    return { query: queryObj, sort };
}
