...
export const getAlltestimonailService = async (payload: any) => {
    ...
    // Fetch the filtered and paginated results from the database
    const results = await testimonialModel.find(query)
        .sort(sort)
        .skip(offset)
        .limit(limit)
        .select("-__v"); // Exclude __v field
...
export const getAllFaqService = async (payload: any) => {
    ...
    const offset = (page - 1) * limit
    const { query, sort } = queryBuilder(payload, ['fullName'])
    const totalDataCount = Object.keys(query).length < 1 ? await faqModel.countDocuments() : await faqModel.countDocuments(query)
    const results = await faqModel.find(query).sort(sort).skip(offset).limit(limit).select("-__v")
    if (results.length) return {
        page,
        limit,
        success: true,
